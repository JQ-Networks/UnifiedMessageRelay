...
def from_dummy_thread(func_to_call_from_main_thread):
    global_vars.callback_queue.put(func_to_call_from_main_thread)


def from_main_thread_blocking():
    callback = global_vars.callback_queue.get() #blocks until an item is available
    callback()
...
def from_main_thread_nonblocking():
    while True:
        try:
            callback = global_vars.callback_queue.get(False) #doesn't block
